...
def find_shortest_path(rn, prev_candi_pt, cur_candi_pt, weight):
    ...
    # case 2, on different roads (including opposite roads)
        ...
        try:
            paths.append(get_shortest_path_with_dist(rn, pre_u, cur_v, prev_candi_pt.offset,
                                                     rn[cur_u][cur_v]['length']-cur_candi_pt.offset, heuristic, weight))
        except nx.NetworkXNoPath:
            pass
        # pre_v -> cur_u
        try:
...
def get_shortest_path_with_dist(rn, src, dest, dist_to_src, dist_to_dest, heuristic, weight):
    dist = 0.0
